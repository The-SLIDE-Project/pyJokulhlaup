...
Ice-marginal lakes are bodies of water found dammed by ice or moraines, or trapped within glaciated overdeepenings. Ice-marginal lakes are important because they can act as significant sediment/nutrient accumulation basins \citep{schomacker2010expansion}, buffer the delivery of freshwater to the proglacial system , and accelerate glacier recession/mass loss. In the palaeo-record, particularly large ice-marginal lakes imposed major controls on ice-stream dynamics \citep{stokes2004evolution} and their subsequent drainage precipitated abrupt and widespread changes in ocean circulation and climate \citep{teller1995history,mangerud2004ice,sufke2022arctic}. 
...
We discretise our Isunnguata Sermia domain (Source of original catchment?) using an anistropic mesh with an edge length of XX m close to terminus, dropping to XX km close to the upper margin of our domain at the approximate ELA (at \( H \approx 1600\ \text{m} \)). Bedmachine v5 is used for $z_{\mathrm{b}}$ and $H$ \citep{Morlighem_Et Al_2022}. As climate forcing, we used the 10 km resolution daily Modèle Atmosphérique Régional (MAR, v3.14) runoff and accumlation data \citep{fettweis2017reconstructions} between 2013--2024. An initialisation $U_{\mathrm{b}}$ is taken from the 2021 MEaSUREs annual mosaic \citep{MEASURES}, from which we invert for the basal friction coefficient using an L-curve scheme \citep[following; ][]{wolovick2023regularization}. Because we have an observational constraint on ice-surface velocity, we use a Schoof-style friction law (see Eq.~\ref{eq:frictionSchoof}, Section~\ref{methods:GlaDS}) which has been shown to more faithfully represent ice-dynamics with a GlaDS derived $N$ than the Budd-style friction law used in our simple synthetic experiments \citep{mcarthur2023basal}. Geothermal heat flux is taken from \citep{shapiro2004inferring}, basal melt rates are taken from \cite{karlsson2021first} and surface ice temperature is taken from \citep{ettema2009higher}.

Our GlaDS parameters are given in Tables~\ref{tab:Synth} \& ~\ref{Appendix:GlaDS}, with changes to the specific values intended to reflect the anticipated change in the scale of basal drainage elements relative to our synthetic domain \citep{hill2024improved}. As boundary conditions, we set $\phi$ equivalent to atmospheric pressure at the lowest elevation vertices within a cluster adjacent to a proglacial river at the glacier terminus, and set two ice-contact boundaries at the lowest elevation nodes along the western and eastern lake boundaries. 
