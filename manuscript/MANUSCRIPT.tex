...
Following \cite{kingslake2013modelling} we included a lumped lake model in which lake depth, $l_{\mathrm{h}}$ ($\mathrm{m}$, Fig.~\ref{FIG:1}), evolves according to the continuity equation:
...
The model presented here, the first time a time-evolving ice-marginal lakes has been coupled to a 2D representation of subglacial hydrology, corroborates many of the findings from earlier 1D work (e.g., REF). 


In our synthetic domain stable and recurrent jökulhlaup are a feature of the default case (Fig.~\ref{FIG:2}) and across the majority of sensitivity tests (Fig.~\ref{FIG:3}). The lake gradually fills at a rate governed by the magnitude of the extraglacial input rate, $Q_{\mathrm{in}}$ and floods initiate once lake depth, $l_{\mathrm{h}}$ reaches between 50--80$\%$ of ice thickness, $H$, corresponding to between 55--90$\%$ of ice overburden pressure. Across our sensitivity tests, higher peak flood discharge, $Q_{\mathrm{r}}$ is typically associated with higher $l_{\mathrm{h}}$ and reflects parameter combinations in which the efficiency or capacity of the distributed sheet is reduced (e.g., lower sheet conductivity, $k_{\mathrm{s}}$, lower bump height, $h_{\mathrm{b}}$, higher cavity spacing, $l_{\mathrm{r}}$). Decreasing the relative efficiency of the sheet inhibits leakage from the lake into the distributed system and permits a greater $\nabla\phi$ close to the lake outlet, which once sufficiently steep, then promotes relatively rapid lake drainage via subglacial channels. Increasing the capacity or efficiency of the distributed sheet either prevents lake drainage entirely with $Q_{\mathrm{in}}$ equilibrium with $Q_{\mathrm{r}}$, or limits the lake highstand and ensures a lower lake volume drains more gradually, with a higher proportion of discharge accommodated without recourse to efficient channelisation. Peak $l_{\mathrm{h}}$ is reduced when the efficiency of the channel is increased (high 

The response of the model to more melt is not expected...

Velocity also follows a similar response...


That this model matches well to early work is no surprise, the lake is governed by the same filling/draining equations as in earlier work and the underlying hydrology models are subject to many of the same physical assumptions. The novelty of the work presented here lies in the introduction of two-dimensions to the description of the lake-adjacent glacier. 

We see lateral propogation and channels developing sub-parallel to the angle of surface slope, we see good match to empircal measurements of lake records
